...
for i in range(0,len(data)):
	...
	df_heading=df_heading.append([{"heading_ID":"heading"+str(i),"name":data.iloc[i,8],"label":"heading"}], ignore_index=True)

for i in range(0,12):
	df_time=df_time.append([{"time_ID":"time"+str(i),"name":str(2*i)+"-"+str(2*i+1),"label":"time"}], ignore_index=True)
df_entity.to_csv('graph_new\\entity.csv',sep=',',header=0,index=False)
df_locgrid.to_csv('graph_new\\locgrid.csv',sep=',',header=0,index=False)
df_heading.to_csv('graph_new\\heading.csv',sep=',',header=0,index=False)
df_time.to_csv('graph_new\\time.csv',sep=',',header=0,index=False)